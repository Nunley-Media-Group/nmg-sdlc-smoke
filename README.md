...
greeting_has_brace("Ada{")  # True
greeting_has_brace("Ada}")  # False
...
`greeting_has_brace` checks for a literal `{` in the complete greeting and uses the same invalid-name validation as `greet`; a closing brace alone does not match.
