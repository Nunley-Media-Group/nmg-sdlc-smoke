...
greeting_has_digit("Ada7")  # True
greeting_has_digit("Ada٣")  # True (Unicode decimal digit)
greeting_has_digit("Ada")  # False
...
`greeting_has_digit` detects Unicode decimal digits in the complete greeting and uses the same validation as `greet`; it does not change the CLI.
