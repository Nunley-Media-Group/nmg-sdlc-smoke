# Requirements: Add greeting_has_tilde library helper for review smoke
...
**I want** a public boolean query for a literal tilde in a completed greeting
**So that** I can inspect greeting text without changing it.

## Acceptance Criteria
### AC1: Detect a literal tilde
**Given** a valid name containing `~`
**When** `greeting_has_tilde("Ada~")` is called through the public `nmg_sdlc_smoke` import
**Then** it returns Python `True` for the completed greeting `Hello, Ada~`.

### AC2: Report an absent tilde
**Given** a valid name without `~`
**When** `greeting_has_tilde("Ada")` is called
...
**When** `greeting_has_tilde` is called
**Then** it raises `ValueError("name must not be blank")` as `greet` does.

## Functional Requirements
| ID | Requirement | Priority |
|---|---|---|
| FR1 | Export a pure `greeting_has_tilde(name: str) -> bool` from `nmg_sdlc_smoke`, checking literal `~` membership in `greet(name)`. | Must |
| FR2 | Preserve `greet` validation, existing helpers and CLI behavior. | Must |
