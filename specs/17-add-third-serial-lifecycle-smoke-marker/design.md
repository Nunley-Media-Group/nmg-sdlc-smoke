# Design: Add third serial lifecycle smoke marker
...
|------|--------|----------|
| `LIVE_SMOKE_C.txt` | Create | Entire file is the UTF-8 byte sequence `LIVE_SMOKE_C\n`: token bytes followed by one LF and nothing else. |
| `README.md` | Modify | Under `## Verification Gates`, append the exact sentence defined below after the existing B sentence and preserve unrelated prose. |
| `scripts/__tests__/live-smoke-c.test.mjs` | Create | Resolve repository root from `import.meta.url`, compare marker bytes to the canonical `Buffer`, and assert README contains the documentation sentence. |

---

## Exact-Byte And README Test Contract

Implement `scripts/__tests__/live-smoke-c.test.mjs` as:
...
Append this exact sentence after the existing `LIVE_SMOKE_B.txt` sentence under README `## Verification Gates` (currently the last sentence before `## Commands`):

> The root [`LIVE_SMOKE_C.txt`](LIVE_SMOKE_C.txt) third serial lifecycle smoke marker contains exactly `LIVE_SMOKE_C` followed by one final newline.

Do not modify any other README section. Do not edit the A or B sentences.
...
|-------|---------------------|-----------------|
| Exact artifact | Read root `LIVE_SMOKE_C.txt` as bytes | Equals `Buffer.from('LIVE_SMOKE_C\n', 'utf8')`. |
| Documentation | Read README `## Verification Gates` | Contains the exact sentence above, including the filename link, third-serial description, token, and final-newline contract. |
| Focused positive contract | From `scripts/`, run `npm test -- --runTestsByPath __tests__/live-smoke-c.test.mjs` with the canonical marker and README sentence | Exit 0. |
| Focused drift contract | Temporarily exercise the focused test with the marker missing, changed, lacking its LF, containing an extra LF, and with canonical bytes but the C sentence removed from README; restore canonical files after each probe | Every drift probe exits non-zero. |
