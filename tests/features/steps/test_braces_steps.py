...
scenarios("../add_nmg_smoke_braces_flag.feature")
...
@when(
    "nmg-smoke --braces --parentheses --uppercase --prefix 'ok: ' "
...
def invoke_braces(
    ...
    code = main([
        "--braces", "--parentheses", "--uppercase", "--prefix", "ok: ",
...
@when(
    "nmg-smoke --parentheses --uppercase --prefix 'ok: ' --repeat 2 "
    "--no-newline Ada and nmg-smoke Ada are run without braces"
)
def invoke_without_braces(
    outcomes: list[tuple[int, str, str]], capsys: pytest.CaptureFixture[str]
) -> None:
    for argv in (
        [
