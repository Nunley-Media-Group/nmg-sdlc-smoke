...
scenarios("../add_greeting_is_ascii_library_function.feature")
...
def library_importable() -> None:
    ...
    assert callable(greeting_is_ascii)
...
def greeting_is_ascii_ada(context: dict[str, object]) -> None:
    context["result"] = greeting_is_ascii("Ada")
...
@then("that value equals greet Ada isascii which is Hello, Ada")
def equals_ada_greeting_is_ascii(context: dict[str, object]) -> None:
    assert greet("Ada") == "Hello, Ada"
    assert context["result"] is True
    assert context["result"] == greet("Ada").isascii()


@when("greeting_is_ascii is called with É")
def greeting_is_ascii_non_ascii(context: dict[str, object]) -> None:
    context["result"] = greeting_is_ascii("É")


@then("it returns False")
def returns_false(context: dict[str, object]) -> None:
    assert context["result"] is False


@then("that value equals greet É isascii which is Hello, É")
def equals_non_ascii_greeting_is_ascii(context: dict[str, object]) -> None:
    assert greet("É") == "Hello, É"
    assert context["result"] is False
    assert context["result"] == greet("É").isascii()


@then("the result is not hardcoded to the Ada result")
def differs_from_ada_result(context: dict[str, object]) -> None:
    assert context["result"] is not True
    assert context["result"] != greeting_is_ascii("Ada")


@when("greeting_is_ascii is called with a blank, whitespace-only, or non-string name")
def greeting_is_ascii_invalid_names(context: dict[str, object]) -> None:
