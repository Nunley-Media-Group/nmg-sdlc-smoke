# Generated from: specs/58-add-nmg-smoke-no-newline-flag/requirements.md
Feature: Add nmg-smoke --no-newline flag
  ...
  @SCN001
  Scenario: Omit final newline on success
  ...
  @SCN002
  Scenario: Omitting the flag keeps the trailing newline
    Given the distribution is installed with its console script
    When nmg-smoke Ada is run
    Then the process exits 0 and prints Hello, Ada followed by a single newline

  @SCN003
  Scenario: Repeated greetings keep separating newlines
    Given the distribution is installed with its console script
  ...
  @SCN004
  Scenario: Uppercase composition still omits only the final newline
    Given the distribution is installed with its console script
  ...
  @SCN007
  Scenario: Library APIs are unchanged
    ...
    And invalid names retain the existing name must not be blank ValueError
