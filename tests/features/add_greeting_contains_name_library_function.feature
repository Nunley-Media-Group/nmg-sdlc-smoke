# File: tests/features/add_greeting_contains_name_library_function.feature
# Generated from: specs/62-add-greeting-contains-name-library-function/requirements.md
Feature: Add greeting_contains_name library function
  ...
  Scenario: Valid name returns True
    ...
    And that value equals Ada in greet Ada which is Hello, Ada
    And the return value is the Python bool True, not the string True

  @SCN002
  Scenario: A different valid name also returns True from membership
    Given the library is importable
    When greeting_contains_name is called with Jo
    Then it returns True
    And that value equals Jo in greet Jo which is Hello, Jo
